      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex justify-center mb-4">
          <div className="inline-flex bg-background border border-border rounded-lg p-1">
            <button className={`px-6 py-2 rounded-md text-sm font-medium transition-colors ${mode === 'encode' ? 'bg-primary text-primary-foreground' : 'text-muted hover:text-foreground'}`} onClick={() => {
            setMode('encode');
            processText(input, 'encode');
          }}>
              Encode to Base64
            </button>
            <button className={`px-6 py-2 rounded-md text-sm font-medium transition-colors ${mode === 'decode' ? 'bg-primary text-primary-foreground' : 'text-muted hover:text-foreground'}`} onClick={() => {
            setMode('decode');
        ...
        <div className="flex flex-col lg:flex-row gap-6 h-[400px]">
          ...
          <div className="flex-1 flex flex-col glass-card p-0 overflow-hidden border border-border">
            ...
            <textarea className="w-full h-full p-4 bg-background resize-none focus:outline-none focus:ring-0 border-none font-mono text-sm leading-relaxed" placeholder={mode === 'encode' ? 'Type or paste plain text here...' : 'Paste Base64 string here...'} value={input} onChange={e => processText(e.target.value, mode)} spellCheck={false}></textarea>
          </div>

          {/* Swap Button (Desktop) */}
          <div className="hidden lg:flex flex-col justify-center">
            <button onClick={toggleMode} className="p-3 bg-surface border border-border rounded-full hover:bg-primary hover:text-white hover:border-primary transition-all text-muted" title="Swap">
          ...
          <div className="lg:hidden flex justify-center -my-3 z-10">
            <button onClick={toggleMode} className="p-3 bg-surface border border-border rounded-full hover:bg-primary hover:text-white hover:border-primary transition-all text-muted">
              <ArrowDownUp size={20} />
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col glass-card p-0 overflow-hidden border border-border">
            ...
            {error ? <div className="h-full p-6 bg-danger/10 text-danger flex flex-col gap-3 font-mono text-sm">
                <p>{error}</p>
              </div> : <textarea className="w-full h-full p-4 bg-foreground/5 text-foreground resize-none focus:outline-none focus:ring-0 border-none font-mono text-sm leading-relaxed" placeholder="Result will appear here..." value={output} readOnly spellCheck={false}></textarea>}
          </div>
        </div>
