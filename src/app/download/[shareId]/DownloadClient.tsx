...
interface DownloadClientProps {
  paramsPromise: Promise<{ shareId: string }>;
}
...
export default function DownloadClient({ paramsPromise }: DownloadClientProps) {
  const { shareId } = use(paramsPromise);
  ...
  const [downloadUrls, setDownloadUrls] = useState<{name: string, downloadUrl: string}[] | null>(null);
  
  const [decryptionKey, setDecryptionKey] = useState<CryptoKey | null>(null);
  const [activeDownload, setActiveDownload] = useState<{ name: string, status: string } | null>(null);
  ...
  const unlockAndDownload = async () => {
    ...
    try {
      ...
      // E2EE Key Extraction
      const { importKeyFromBase64, deriveKeyFromPassword, decryptFileKeyWithKEK } = await import('@/lib/crypto');
  ...
  const downloadAndDecrypt = async (fileUrl: string, fileName: string) => {
    ...
    setActiveDownload({ name: fileName, status: "Downloading encrypted blob..." });
    ...
    try {
      ...
      setActiveDownload({ name: fileName, status: "Decrypting locally..." });
  ...
  if (loading) {
    return (
      <ToolLayout
        ...
        description="Retrieving transfer information..."
        breadcrumbs={[{ label: "Download", href: "#" }]}
      >
        <div className="card max-w-2xl mx-auto p-12 glass-card text-center">
          <div className="animate-spin text-primary mx-auto mb-4" style={{ width: "40px", height: "40px", border: "3px solid transparent", borderTopColor: "currentColor", borderRadius: "50%" }}></div>
        </div>
      </ToolLayout>
    );
  }
  ...
  if (error && !info) {
    return (
      <ToolLayout
        ...
        description="This transfer could not be found or has expired."
        breadcrumbs={[{ label: "Download", href: "#" }]}
      >
  ...
  return (
    <ToolLayout
      title="Secure Download"
      description={`Download your file securely from Filoza.`}
      breadcrumbs={[{ label: "Download", href: "#" }]}
    >
      <div className="card max-w-2xl mx-auto p-6 md:p-8 glass-card">
        {error && (
          <div className="bg-danger bg-opacity-10 text-danger p-4 rounded-lg mb-6 flex items-center gap-3">
                ...
                return (
                  <div key={idx} className="flex justify-between items-center p-3 rounded-lg" style={{ backgroundColor: "rgba(255,255,255,0.4)", border: "1px solid var(--border)" }}>
                    ...
                    <div className="flex items-center gap-4 flex-shrink-0">
                      ...
                      {dl && decryptionKey && (
                        <button 
                          onClick={() => downloadAndDecrypt(dl.downloadUrl, file.name)}
                          disabled={!!activeDownload}
                          className="btn btn-primary btn-sm flex items-center gap-1"
                        >
                          {isDownloadingThis ? (
                            <div className="animate-spin text-white" style={{ width: "14px", height: "14px", border: "2px solid transparent", borderTopColor: "currentColor", borderRadius: "50%" }}></div>
                          ) : (
                            <Download size={14} /> 
                          )}
                          <span className="hidden sm:inline">{isDownloadingThis ? "Decrypting..." : "Decrypt & Save"}</span>
                        </button>
                      )}
                    </div>
                  </div>
        ...
        {activeDownload && (
          <div className="mb-8 bg-[var(--background)] p-4 rounded-xl border border-[var(--border)] text-center">
        ...
        {!downloadUrls ? (
          <div className="bg-secondary p-6 rounded-xl text-center">
            {info?.isProtected ? (
              <>
                ...
                <input 
                  type="password" 
                  className="input w-full max-w-sm mx-auto mb-4" 
            ...
            <button 
              className="btn btn-primary w-full max-w-sm mx-auto flex items-center justify-center gap-2"
          ...
          <div className="bg-success bg-opacity-10 text-success p-6 rounded-xl text-center">
