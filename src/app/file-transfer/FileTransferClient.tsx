...
export default function FileTransferClient() {
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      const { generateFileKey, exportKeyToBase64, deriveKeyFromPassword, encryptFileKeyWithKEK, encryptFile, hashPasswordForBackend } = await import('@/lib/crypto');
      ...
      const { shareId: newShareId, uploadUrls } = data;
  ...
  const shareUrl = shareId ? `${typeof window !== 'undefined' ? window.location.origin : ''}/download/${shareId}` : '';
  const totalSize = files.reduce((acc, f) => acc + f.size, 0);
    ...
    >
      {!shareId ? (
        <div className="card max-w-3xl mx-auto p-6 glass-card">
          ...
          {files.length === 0 ? (
            <label 
              className="dropzone block cursor-pointer relative" 
              onDrop={handleDrop} 
            ...
            <>
              <div className="mb-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {!isUploading && (
                    <label className="text-primary text-sm hover:underline cursor-pointer m-0 relative">
                      + Add more
              ...
              {!isUploading && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 pt-6" style={{ borderTop: "1px solid var(--border)" }}>
                  <div>
              ...
              {isUploading ? (
                <div className="mb-4">
                  ...
                  <div className="w-full bg-secondary rounded-full h-2.5 overflow-hidden">
                    <div className="bg-primary h-2.5 transition-all duration-300 ease-out" style={{ width: `${uploadProgress}%` }}></div>
                  </div>
                </div>
              ) : (
                <div className="flex gap-4 justify-end">
                  ...
                  <button className="btn btn-primary flex items-center gap-2" onClick={startUpload}>
                    <Send size={18} /> Send Files
                  </button>
                </div>
              )}
            </>
