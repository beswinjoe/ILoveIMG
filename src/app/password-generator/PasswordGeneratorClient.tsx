...
export default function PasswordGeneratorClient() {
  ...
  const [strength, setStrength] = useState<{ label: string; color: string; score: number }>({ label: "Strong", color: "text-success", score: 4 });
  ...
  const calculateStrength = (pwd: string) => {
    let score = 0;
    if (!pwd) return { label: "Weak", color: "text-danger", score: 0 };
    ...
    if (score < 3) return { label: "Weak", color: "text-danger", score };
    if (score < 5) return { label: "Fair", color: "text-warning", score };
    if (score < 7) return { label: "Good", color: "text-primary", score };
  ...
  const generatePassword = () => {
    ...
    if (charset === "") {
      setPassword("");
      setStrength({ label: "None", color: "text-muted", score: 0 });
      return;
    }
          ...
          <div className="relative mb-6">
            ...
            {password && (
              <button 
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 p-3 text-muted hover:text-foreground transition-colors hover:bg-surface rounded-md"
              ...
              <div className="flex items-center gap-2">
                <div className="flex gap-1">
                  {[...Array(4)].map((_, i) => (
                    <div 
                      key={i} 
                      className={`h-2 w-8 rounded-full ${i < Math.ceil(strength.score / 2) ? (
                        strength.label === 'Weak' ? 'bg-danger' : 
                        strength.label === 'Fair' ? 'bg-warning' : 
                        strength.label === 'Good' ? 'bg-primary' : 'bg-success'
                      ) : 'bg-border'}`}
                    />
                  ))}
                </div>
