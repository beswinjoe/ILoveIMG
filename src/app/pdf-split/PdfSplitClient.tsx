            ...
            <div className="flex flex-col gap-4 mb-8 p-6 bg-background rounded-lg border border-border">
              ...
              <div className="flex flex-col gap-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="radio" name="splitMode" value="extract_all" checked={splitMode === "extract_all"} onChange={() => setSplitMode("extract_all")} className="w-4 h-4 text-primary" />
                  <span>Extract all pages into separate PDFs (Downloads as ZIP)</span>
                </label>
                
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="radio" name="splitMode" value="custom_ranges" checked={splitMode === "custom_ranges"} onChange={() => setSplitMode("custom_ranges")} className="w-4 h-4 text-primary" />
                  <span>Custom page ranges</span>
                </label>
              </div>

              {splitMode === "custom_ranges" && <div className="mt-4 pl-7">
                  <label className="label">Pages (e.g., 1-5, 8, 11-13)</label>
                  <input type="text" className="input max-w-md" value={customRange} onChange={e => setCustomRange(e.target.value)} placeholder="1-5, 8, 11-13" />
                </div>}
            </div>
