...
export default function PdfToImagesClient() {
  ...
  const convertPdfToImages = async () => {
    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
        ...
        zip.file(`page_${pageNum}.${extension}`, base64Data, { base64: true });
  ...
  return (
    <ToolLayout
      ...
      relatedTools={[
        { name: "Image to PDF", href: "/image-to-pdf", icon: <CheckCircle2 /> },
        { name: "JPG to PNG", href: "/jpg-to-png", icon: <CheckCircle2 /> }
      ]}
    >
            ...
            {isProcessing && totalPages > 0 && (
              <div className="mb-6">
                ...
                <div className="w-full bg-border rounded-full h-2.5">
                  <div className="bg-primary h-2.5 rounded-full transition-all duration-300" style={{ width: `${(progress / totalPages) * 100}%` }}></div>
                </div>
              </div>
            )}
