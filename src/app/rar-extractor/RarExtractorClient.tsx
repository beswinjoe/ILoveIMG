...
export default function RarExtractorClient() {
  ...
  useEffect(() => {
    ...
    import("libarchive.js").then(({ Archive }) => {
      Archive.init({
  ...
  const flattenArchive = async (obj: Record<string, unknown>, path: string = ""): Promise<ExtractedFile[]> => {
    ...
    for (const [key, val] of Object.entries(obj)) {
      if (val instanceof File) {
        ...
        // It's a directory
        const subFiles = await flattenArchive(val, path + key + "/");
  ...
  return (
    <ToolLayout
      title="RAR & 7Z Extractor"
      description="Extract files from RAR, 7Z, and other archives directly in your browser. No uploads, 100% private."
      breadcrumbs={[{ label: "Archive", href: "/#archive" }, { label: "RAR Extractor", href: "/rar-extractor" }]}
      howItWorks={[
        "Drag and drop your .rar or .7z file into the box above.",
        "Wait a moment while we read the archive structure locally.",
        "Browse the contents of the archive.",
        "Click the download button next to any file you want to save."
      ]}
      ...
      relatedTools={[
        { name: "ZIP Creator", href: "/zip-creator", icon: <ArchiveRestore /> },
        { name: "ZIP Extractor", href: "/zip-extractor", icon: <ArchiveRestore /> },
      ]}
    >
      {!archiveFile ? (
        <div className="max-w-2xl mx-auto flex flex-col gap-4">
          ...
          <div 
            className={`dropzone ${!isReady ? 'opacity-50 pointer-events-none' : ''}`}
            onDrop={handleDrop} 
        ...
        <div className="card max-w-4xl mx-auto mt-8 p-6 glass-card">
          ...
          {isProcessing ? (
            ...
            <div className="max-h-[500px] overflow-y-auto pr-2 custom-scrollbar" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {extractedFiles.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center p-4 rounded-lg" style={{ backgroundColor: "rgba(255,255,255,0.4)", border: "1px solid var(--border)" }}>
