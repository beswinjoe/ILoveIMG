...
const categories: Record<UnitCategory, {
  id: string;
  name: string;
  ...
  data: [{
    ...
    id: "mb",
    name: "Megabytes (MB)",
    baseMultiplier: 1048576
  }, {
    id: "gb",
    name: "Gigabytes (GB)",
    baseMultiplier: 1073741824
  }, {
    id: "tb",
    name: "Terabytes (TB)",
    baseMultiplier: 1099511627776
  }]
};
export default function UnitConverterClient() {
  ...
  useEffect(() => {
    ...
    if (category === "temperature") {
      ...
      if (fromUnit === "c") celsius = val;else if (fromUnit === "f") celsius = (val - 32) * 5 / 9;else if (fromUnit === "k") celsius = val - 273.15;

      // Convert from Celsius to Target
      if (toUnit === "c") result = celsius;else if (toUnit === "f") result = celsius * 9 / 5 + 32;else if (toUnit === "k") result = celsius + 273.15;
    } else {
        ...
        <div className="glass-card flex flex-col gap-8">
          ...
          <div className="flex justify-center">
            <div className="inline-flex bg-background border border-border rounded-lg p-1 overflow-x-auto w-full sm:w-auto">
              {(Object.keys(categories) as UnitCategory[]).map(cat => <button key={cat} className={`px-4 sm:px-6 py-2 rounded-md text-sm font-medium transition-colors capitalize whitespace-nowrap ${category === cat ? 'bg-primary text-primary-foreground' : 'text-muted hover:text-foreground'}`} onClick={() => setCategory(cat)}>
                  {cat}
                </button>)}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex-1 w-full bg-background p-4 rounded-xl border border-border">
              ...
              <div className="flex flex-col gap-3">
                <input type="number" className="input text-2xl font-semibold bg-transparent border-none p-0 focus:ring-0" value={fromValue} onChange={e => setFromValue(e.target.value)} placeholder="0" />
                <select className="input w-full bg-surface" value={fromUnit} onChange={e => setFromUnit(e.target.value)}>
                  {categories[category].map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                </select>
              </div>
            </div>

            {/* Swap Button */}
            <button onClick={handleSwap} className="p-3 bg-surface border border-border rounded-full hover:bg-primary hover:text-white hover:border-primary transition-all text-muted shrink-0">
            ...
            <div className="flex-1 w-full bg-background p-4 rounded-xl border border-border">
              ...
              <div className="flex flex-col gap-3">
                <input type="text" className="input text-2xl font-semibold bg-transparent border-none p-0 focus:ring-0 text-primary" value={toValue} readOnly placeholder="0" />
                <select className="input w-full bg-surface" value={toUnit} onChange={e => setToUnit(e.target.value)}>
                  {categories[category].map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                </select>
              </div>
            </div>
