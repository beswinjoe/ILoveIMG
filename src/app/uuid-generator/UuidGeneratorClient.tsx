            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input type="range" min="1" max="100" value={count} onChange={e => setCount(parseInt(e.target.value))} className="flex-1" />
              ...
              <div>
                ...
                <select className="input w-full bg-background" value={version} onChange={e => setVersion(e.target.value as any)}>
        ...
        <div className="flex-1">
          <div className="glass-card h-full flex flex-col">
            ...
            <div className="bg-background rounded-lg border border-border overflow-hidden flex-1 max-h-[600px] overflow-y-auto">
              {uuids.map((uuid, index) => <div key={index} className="flex justify-between items-center p-3 border-b border-border hover:bg-surface/50 group transition-colors">
                  <span className="font-mono text-sm sm:text-base break-all pr-4">{uuid}</span>
                  <button onClick={() => copyToClipboard(uuid, index)} className="p-2 text-muted hover:text-foreground hover:bg-surface rounded opacity-0 group-hover:opacity-100 transition-all flex-shrink-0" title="Copy UUID">
