...
export default function ZipCreatorClient() {
  ...
  return <>
      {!zipUrl && files.length === 0 ? <div className="dropzone max-w-2xl mx-auto" onDrop={handleDrop} onDragOver={e => e.preventDefault()} onClick={() => fileInputRef.current?.click()}>
        ...
        </div> : <div className="card max-w-3xl mx-auto mt-8 p-6 glass-card">
        ...
        borderBottom: "1px solid var(--border)"
          ...
          {!zipUrl ? <>
              <div className="max-h-[300px] overflow-y-auto mb-6 pr-2 custom-scrollbar" style={{
          display: 'flex',
          ...
          gap: '0.5rem'
            ...
            border: "1px solid var(--border)"
                    ...
                    <div className="flex items-center gap-4 flex-shrink-0">
                      ...
                      <button onClick={() => removeFile(idx)} className="text-muted hover:text-danger transition-colors p-1" aria-label="Remove file" disabled={isProcessing}>
                        <X size={16} />
                      </button>
                    </div>
                  </div>)}
              ...
              {isProcessing && <div className="mb-6">
              ...
              <div className="flex justify-center gap-4 pt-4" style={{
          borderTop: "1px solid var(--border)"
            ...
            display: "none"
                ...
                <button className="btn btn-secondary text-danger hover:bg-danger hover:bg-opacity-10" onClick={clearFiles} disabled={isProcessing}>
                  Clear All
                </button>
                <button className="btn btn-primary ml-auto flex items-center gap-2" onClick={createZip} disabled={isProcessing || files.length === 0}>
                  <FileArchive size={18} />
